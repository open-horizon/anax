		...
		if len(topics) != 0 {
			...
			if results, err := read(time.Duration(10)*time.Second, 30); err != nil {
				...
				for _, r := range results {
					...
					// Send the whisper message to the internal message bus if the msg is for me
					glog.V(5).Infof("Ignoring messages not for me: %v", me)
					if r.To == me {
						wm := events.NewWhisperReceivedMessage(events.RECEIVED_MSG, r.Payload, r.From, r.To)
						w.Messages() <- wm
					}
