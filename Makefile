...
CSS_IMAGE_VERSION ?= 1.6.4$(BRANCH_NAME)
...
ESS_IMAGE_VERSION ?= 1.6.4$(BRANCH_NAME)
...
anax-image:
	...
	bash image_bld.sh -c anax -n $(ANAX_IMAGE) -l $(ANAX_IMAGE_LABELS) -d $(DOCKER_MAYBE_CACHE) -s $(ANAX_IMAGE_STG) -a $(arch)
agbot-image:
	@echo "Producing agbot docker image $(AGBOT_IMAGE)"
	bash image_bld.sh -c agbot -n $(AGBOT_IMAGE) -l $(AGBOT_IMAGE_LABELS) -d $(DOCKER_MAYBE_CACHE) -s $(AGBOT_IMAGE_STG) -a $(arch)
...
promote-anax:
	...
	bash promote_image.sh -n $(ANAX_IMAGE) -p $(ANAX_IMAGE_PROD) -l $(ANAX_IMAGE_LATEST)

# you must set ANAX_IMAGE_VERSION to the correct version for promotion to production
promote-agbot:
	@echo "Promoting $(AGBOT_IMAGE)"
	bash promote_image.sh -n $(AGBOT_IMAGE) -p $(AGBOT_IMAGE_PROD) -l $(AGBOT_IMAGE_LATEST)
promote-anax-k8s:
	@echo "Promoting $(ANAX_K8S_IMAGE)"
	bash promote_image.sh -n $(ANAX_K8S_IMAGE) -p $(ANAX_K8S_IMAGE_PROD) -l $(ANAX_K8S_IMAGE_LATEST)
...
anax-k8s-image: anax-k8s-clean
	...
	bash image_bld.sh -c anax-k8s -n $(ANAX_K8S_IMAGE) -l $(ANAX_K8S_IMAGE_LABELS) -d $(DOCKER_MAYBE_CACHE) -s $(ANAX_K8S_IMAGE_STG) -a $(arch)
...
css-docker-image: css-clean
	...
	bash image_bld.sh -c css-docker -n $(CSS_IMAGE) -l $(CSS_IMAGE_LABELS) -d $(DOCKER_MAYBE_CACHE) -s $(CSS_IMAGE_STG) -a $(arch)
promote-css:
	@echo "Promoting $(CSS_IMAGE)"
	bash promote_image.sh -n $(CSS_IMAGE) -p $(CSS_IMAGE_PROD) -l $(CSS_IMAGE_LATEST)
ess-docker-image: ess-clean
	@echo "Producing ESS docker image $(ESS_IMAGE)"
	bash image_bld.sh -c ess-docker -n $(ESS_IMAGE) -l $(ESS_IMAGE_LABELS) -d $(DOCKER_MAYBE_CACHE) -s $(ESS_IMAGE_STG) -a $(arch)

ess-promote:
	@echo "Promoting $(ESS_IMAGE)"
	bash promote_image.sh -n $(ESS_IMAGE) -p $(ESS_IMAGE_PROD) -l $(ESS_IMAGE_LATEST)
