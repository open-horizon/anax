...
func (d DeploymentConfigRegistry) StartTest(homeDirectory string, userInputFile string, configFiles []string, configType string, noFSS bool) error {
	for _, p := range d {
		if owned := p.StartTest(homeDirectory, userInputFile, configFiles, configType, noFSS); owned {
			return nil
		}
	}
