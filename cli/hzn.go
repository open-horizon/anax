...
`))
	...
	userinputCompNodeArch := userinputCompCmd.Flag("arch", msgPrinter.Sprintf("The architecture of the node. It is required when -n is not specified is specified. If omitted, the service of all the architectures referenced in the business policy will be checked for compatibility.")).Short('a').String()
