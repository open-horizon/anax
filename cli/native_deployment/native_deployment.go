...
var VALID_DEPLOYMENT_FIELDS = map[string]int8{"image": 1, "privileged": 1, "cap_add": 1, "environment": 1, "devices": 1, "binds": 1, "specific_ports": 1, "command": 1, "ports": 1, "ephemeral_ports": 1, "tmpfs": 1, "network": 1, "entrypoint": 1, "max_memory_mb": 1, "max_cpus": 1}
...
func (p *NativeDeploymentConfigPlugin) StartTest(homeDirectory string, userInputFile string, configFiles []string, configType string, noFSS bool, userCreds string) bool {
	...
	fullSvcName := serviceDef.Org + "/" + serviceDef.URL
	msNetworks, perr := dev.ProcessStartDependencies(dir, fullSvcName, deps, userInputs.Global, userInputs.Services, cw)
	if perr != nil {
		if !noFSS {
			sync_service.Stop(cw.GetClient())
		}
