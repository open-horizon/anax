...
func (pm *BaseProtocolMessage) IsValid() bool {
    return pm.msgType != "" && pm.protocol != "" && pm.agreementId != ""
}

func (pm *BaseProtocolMessage) String() string {
    return fmt.Sprintf("Type: %v, Protocol: %v, Version: %v, AgreementId: %v", pm.msgType, pm.protocol, pm.version, pm.agreementId)
}

func (pm *BaseProtocolMessage) ShortString() string {
    return fmt.Sprintf("Type: %v, AgreementId: %v", pm.msgType, pm.agreementId)
}
...
func SendProposal(p ProtocolHandler,
    ...
    if err := p.PolicyManager().AttemptingAgreement(consumerPolicy, newProposal.AgreementId()); err != nil {
        glog.Errorf(fmt.Sprintf("Protocol %v error saving agreement count: %v", p.Name(), err))
    }
    ...
    if err := SendProtocolMessage(messageTarget, newProposal, sendMessage); err != nil {
        ...
        if perr := p.PolicyManager().CancelAgreement(consumerPolicy, newProposal.AgreementId()); perr != nil {
            glog.Errorf(fmt.Sprintf("Protocol %v error saving agreement count: %v", p.Name(), perr))
        }
...
func DecideOnProposal(p ProtocolHandler,
                    proposal Proposal,
                    myId string) (*BaseProposalReply, error) {
    ...
    if replyErr != nil {
        ...
        if producerPolicy != nil {
            if cerr := p.PolicyManager().CancelAgreement(producerPolicy, proposal.AgreementId()); cerr != nil {
                glog.Errorf(fmt.Sprintf("Error cancelling agreement %v in PM %v", proposal.AgreementId(), cerr))
            }
        }
        return nil, replyErr
    }
...
func SendProtocolMessage(messageTarget interface{}, msg interface{}, sendMessage func(mt interface{}, pay []byte) error) error {
