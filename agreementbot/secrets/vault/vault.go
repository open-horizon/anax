...
func (vs *AgbotVaultSecrets) ListOrgUserSecrets(user, password, org string) ([]string, error) {

	glog.V(3).Infof(vaultPluginLogString(fmt.Sprintf("listing secrets for user %v in %v", user, org)))

	_, userId := cutil.SplitOrgSpecUrl(user)
	url := fmt.Sprintf("%s/v1/openhorizon/%s/user/%s?list=true", vs.cfg.GetAgbotVaultURL(), org, userId)
