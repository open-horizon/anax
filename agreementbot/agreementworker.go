...
func NewCSAgreementWorker(policyManager *gpolicy.PolicyManager, config *config.HorizonConfig, db *bolt.DB) *CSAgreementWorker {
...
type CSInitiateAgreement struct {
    workType        string
    ProducerPolicy *gpolicy.Policy     // the producer policy received from the exchange
    ConsumerPolicy *gpolicy.Policy     // the consumer policy we're matched up with
    ...
    for {
        ...
        if workItem.Type() == INITIATE {
            ...
            // Initiate the protocol
                ...
                // Publish error on the message bus
                // TODO:
            ...
            if reply, err := protocolHandler.ValidateReply(wi.Reply); err != nil {
                ...
                // New we need to write the info to the exchange
                ...
                } else if pol, err := gpolicy.DemarshalPolicy(proposal.TsAndCs); err != nil {
                ...
                // We need to write the info to the blockchain
                    ...
                    // Need to cancel agreement
                    if err := DeleteAgreement(a.db, reply.AgreementId()); err != nil {
                        glog.Errorf(logString(fmt.Sprintf("error deleting pending agreement: %v, error: %v", reply.AgreementId(), err)))
                    }
...
func (a *CSAgreementWorker) recordConsumerAgreementState(agreementId string, workloadID string, state string) error {
    ...
    for {
        if err, tpErr := exchange.InvokeExchange(a.httpClient, "PUT", targetURL, &as, &resp); err != nil {
            ...
            glog.V(5).Infof("ExchangeProcessor set agreement %v to state %v", agreementId, state)
            return nil
        }
    }
