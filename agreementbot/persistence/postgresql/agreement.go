...
const AGREEMENT_CREATE_DEVICE_ID_INDEX = `CREATE INDEX IF NOT EXISTS "device_id_agreements_ ON "agreements_ USING GIN ((agreement -> 'device_id'));`
...
func (db *AgbotPostgresqlDB) GetPrimaryAgreementPartitionTableDeviceIdIndexCreate() string {
	sql := strings.Replace(AGREEMENT_CREATE_DEVICE_ID_INDEX, AGREEMENT_TABLE_NAME_ROOT, db.GetAgreementPartitionTableName(db.PrimaryPartition()), 2)
	return sql
}
...
func (db *AgbotPostgresqlDB) FindAgreementsPage(filters []persistence.AgbotDBFilter, protocol string, nextId string, limit int) (string, []persistence.Agreement, error) {
	...
	for _, currentPartition := range db.AllPartitions() {
		...
		glog.V(3).Infof("Find agreements using SQL: %v for partition %v, protocol: %v, nextId: %v, limit: %v", sqlStr, currentPartition, protocol, nextId, limit)
