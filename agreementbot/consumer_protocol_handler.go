...
func (b *BaseConsumerProtocolHandler) CancelAgreement(ag persistence.Agreement, reason string, cph ConsumerProtocolHandler) {
	glog.V(5).Infof(BCPHlogstring(b.Name(), fmt.Sprintf("Lily - CancelAgreement: %v, reason: %v", ag, reason)))
	...
	if wlUsage, err := b.db.FindSingleWorkloadUsageByDeviceAndPolicyName(ag.DeviceId, ag.PolicyName); err != nil {
		...
		if theDev != nil && theDev.HAGroup != "" {
			...
			if err != nil {
				...
				// return
				// set the updatePendingUpgrade
			}
			glog.V(5).Infof(BCPHlogstring(b.Name(), fmt.Sprintf("Lily - In CancelAgreement, ha partners for %v are: %v", ag.DeviceId, partners)))
			...
			// upgradingWorkload == nil || (upgradingWorkload != nil && anotherWorkload != nil & anotherWorkload.PendingUpgradeTime == 0), means no workload is upgrading currently for this hagroup
			...
			if err = b.db.InsertHAUpgradingWorkloadForGroupAndPolicy(deviceAndGroupOrg, theDev.HAGroup, ag.PolicyName, ag.DeviceId); err != nil {
				...
				glog.V(5).Infof(BCPHlogstring(b.Name(), fmt.Sprintf("Lily - In CancelAgreement, delete workloadusage and cancel agreement for: org: %v, hagroup: %v, policyName: %v deviceId: %v", ag.Org, theDev.HAGroup, ag.PolicyName, ag.DeviceId)))
				if err := b.db.DeleteWorkloadUsage(ag.DeviceId, ag.PolicyName); err != nil {
					glog.Warningf(BCPHlogstring(b.Name(), fmt.Sprintf("error deleting workload usage for %v using policy %v, error: %v", ag.DeviceId, ag.PolicyName, err)))
				}
	...
	glog.V(5).Infof(BCPHlogstring(b.Name(), fmt.Sprintf("Lily - In CancelAgreement, delete non-HA workloadusage and cancel agreement for: org: %v, policyName: %v deviceId: %v", ag.Org, ag.PolicyName, ag.DeviceId)))
