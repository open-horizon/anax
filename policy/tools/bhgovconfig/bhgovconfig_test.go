...
func runCmd(commandString, stdinFilename string, args ...string) ([]byte, []byte, error) {
    ...
    if cmd == nil { return nil, nil, errors.New("Did not return a command object, returned nil") }
    // Create the std in pipe, this is how govconfig.json gets passed into the command
    stdin, err := cmd.StdinPipe()
    if err != nil { return nil, nil, errors.New("Could not get Stdin pipe, error: " + err.Error()) }
    // Create the stdout pipe to hold the output from the command
    stdout, err := cmd.StdoutPipe()
    if err != nil { return nil, nil, errors.New("Error retrieving output from command, error: " + err.Error()) }
    // Create the stderr pipe to hold the errors from the command
    stderr, err := cmd.StderrPipe()
    if err != nil { return nil, nil, errors.New("Error retrieving stderr from command, error: " + err.Error()) }
    // Read the input file
    jInbytes, err := ioutil.ReadFile(stdinFilename)
    if err != nil { return nil, nil, errors.New("Unable to read " + stdinFilename + " file, error: " + err.Error()) }
    // Start the command, which will block for input to std in
    err = cmd.Start()
    if err != nil { return nil, nil, errors.New("Unable to start command, error: " + err.Error()) }
    // Send in the std in bytes
    _, err = stdin.Write(jInbytes)
    if err != nil { return nil, nil, errors.New("Unable to write to stdin of command, error: " + err.Error()) }
    // Close std in so that the command will begin to execute
    err = stdin.Close()
    if err != nil { return nil, nil, errors.New("Unable to close stdin, error: " + err.Error()) }
    err = error(nil)
    // Read the output from stdout and stderr into byte arrays
    // stdoutBytes, err := readPipe(stdout)
    stdoutBytes, err := ioutil.ReadAll(stdout)
    if err != nil { return nil, nil, errors.New("Error reading stdout, error: " + err.Error()) }
    // stderrBytes, err := readPipe(stderr)
    stderrBytes, err := ioutil.ReadAll(stderr)
    if err != nil { return nil, nil, errors.New("Error reading stderr, error: " + err.Error()) }
    // Now block waiting for the command to complete
    err = cmd.Wait()
    if err != nil { return stdoutBytes, stderrBytes, errors.New("Error waiting for command: " + err.Error()) }
