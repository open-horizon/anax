...
- `secretBinding`: This section is used to bind secret names defined in the servcie with the secret names in the vault. The secret value will be retrived from the vault and passed to the service container at the deployment time. The secret value is used by the service container to access other applications.
  - `serviceUrl`: The name of the service. It can be the top level services defined in the `services` attribute or their dependent services. This is the same value as found in the `url` field [here](./service_def.md).
  - `serviceOrgid`: The organization in which the service in `serviceUrl` is defined.
  - `serviceArch`: The hardware architecture of the service in `serviceUrl`, or `*` to indicate any compatible architecture. This is the same value as found in the `arch` field [here](./service_def.md).
  ...
  - `secrets`: A list of secret bindings. Each elelment is a map of string keyed by the name of the secret in the service. The value is the name of the secret in the vault. The valid formats for the vault secret names are: `<secretname>` for the organization level secret; `user/<username>/<secretname>` for the user level secret. The fully qualified vault secret name will be the the value defined here preceded by `openhorizon/<orgname>/` where the `<orgname>` is the organzation name of this deployment policy. However, it is invalid to specify the fully qualified name here.
...
Both `2.3.0` and `2.3.1` versions of the services have a secret `ai_secret` defined that the service container will use to access an AI service on the clound once the vault secret name is bound to it. The policy binds it to a vault secret named `cloud_ai_secret_name`.
```
{
