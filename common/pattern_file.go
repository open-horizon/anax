...
type AbstractPatternFile interface {
	...
	GetSecretBinding() []exchange.SecretBinding
}
...
type PatternFile struct {
	...
	SecretBinding      []exchange.SecretBinding     `json:"secretBinding,omitempty"`
}
...
func (p *PatternFile) GetSecretBinding() []exchange.SecretBinding {
...
func (p *PatternFile) VerifySecretBinding(serviceDefResolverHandler exchange.ServiceDefResolverHandler, msgPrinter *message.Printer) error {
	...
	if p.Services == nil || len(p.Services) == 0 {
		if p.SecretBinding == nil || len(p.SecretBinding) == 0 {
			...
			return fmt.Errorf(msgPrinter.Sprintf("None of the services in pattern %v defines secret. These secret binding are not needed: %v.", p.Name, p.SecretBinding))
		}
	}
	...
	for _, svc := range p.Services {
		if svc.ServiceVersions != nil {
			for _, v := range svc.ServiceVersions {
				if err := ValidateSecretBinding(p.SecretBinding, svc.ServiceOrg, svc.ServiceURL, v.Version, svc.ServiceArch, serviceDefResolverHandler, msgPrinter); err != nil {
					return err;
				}
			}
		}
