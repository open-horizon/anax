...
do
   if [ "${EXCH_APP_HOST}" = "http://exchange-api:8080/v1" ]; then
     AGID1=$(curl -sS http://localhost:8082/agreement | jq -r '.agreements.active[0].current_agreement_id')
     AGID2=$(curl -sS http://localhost:8082/agreement | jq -r '.agreements.active[1].current_agreement_id')
     AGID3=$(curl -sS http://localhost:8082/agreement | jq -r '.agreements.active[2].current_agreement_id')
     AGID4=$(curl -sS http://localhost:8082/agreement | jq -r '.agreements.active[3].current_agreement_id')

     echo "Agbot deleting agreements"
     echo "Deleting $AGID1"
     DEL=$(curl -sS -X DELETE http://localhost:8082/agreement/$AGID1)

     echo "Deleting $AGID2"
     DEL=$(curl -sS -X DELETE http://localhost:8082/agreement/$AGID2)

     echo "Deleting $AGID3"
     DEL=$(curl -sS -X DELETE http://localhost:8082/agreement/$AGID3)

     echo "Deleting $AGID4"
     DEL=$(curl -sS -X DELETE http://localhost:8082/agreement/$AGID4)
   else
