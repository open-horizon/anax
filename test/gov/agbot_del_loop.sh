...
do
   if [ "${REMOTE_HUB}" -eq 0 ]; then
     ...
     echo "Deleting $AGID1"
     curl -sS -X DELETE "${AGBOT_API}/agreement/$AGID1"

     echo "Deleting $AGID2"
     curl -sS -X DELETE "${AGBOT_API}/agreement/$AGID2"

     echo "Deleting $AGID3"
     curl -sS -X DELETE "${AGBOT_API}/agreement/$AGID3"
     ...
     echo "Agbot deleting agreements"
     echo "Deleting $AGID1"
     kubectl -n kube-system exec -ti "$(kubectl -n kube-system get pods | grep -v 'agbot-db' | grep -m 1 'edge-computing-agbot' | awk '{print $1}')" -- sh -c "export ANAX_PORT=\$(cat /etc/horizon/anax.json | jq -r .AgreementBot.APIListen | awk -F: '{print \$NF}'); curl -sS -X DELETE http://localhost:\$ANAX_PORT/agreement/$AGID1"

     echo "Deleting $AGID2"
     kubectl -n kube-system exec -ti "$(kubectl -n kube-system get pods | grep -v 'agbot-db' | grep -m 1 'edge-computing-agbot' | awk '{print $1}')" -- sh -c "export ANAX_PORT=\$(cat /etc/horizon/anax.json | jq -r .AgreementBot.APIListen | awk -F: '{print \$NF}'); curl -sS -X DELETE http://localhost:\$ANAX_PORT/agreement/$AGID2"

     echo "Deleting $AGID3"
     kubectl -n kube-system exec -ti "$(kubectl -n kube-system get pods | grep -v 'agbot-db' | grep -m 1 'edge-computing-agbot' | awk '{print $1}')" -- sh -c "export ANAX_PORT=\$(cat /etc/horizon/anax.json | jq -r .AgreementBot.APIListen | awk -F: '{print \$NF}'); curl -sS -X DELETE http://localhost:\$ANAX_PORT/agreement/$AGID3"

     echo "Deleting $AGID4"
     kubectl -n kube-system exec -ti "$(kubectl -n kube-system get pods | grep -v 'agbot-db' | grep -m 1 'edge-computing-agbot' | awk '{print $1}')" -- sh -c "export ANAX_PORT=\$(cat /etc/horizon/anax.json | jq -r .AgreementBot.APIListen | awk -F: '{print \$NF}'); curl -sS -X DELETE http://localhost:\$ANAX_PORT/agreement/$AGID4"
   fi
