...
while (true) do
  ...
  then
      ...
      if [ ${CERT_LOC} -eq "1" ]; then
        /usr/bin/old-anax -v=5 -alsologtostderr=true -config /etc/colonus/anax-combined.config >/tmp/anax.log 2>&1 > /dev/null
      else
        /usr/bin/old-anax -v=5 -alsologtostderr=true -config /etc/colonus/anax-combined-no-cert.config >/tmp/anax.log 2>&1  > /dev/null
      fi
  else
