...
new_deployment_props="{\"properties\": $(hzn ex dep listpolicy bp_pws -u $USERDEV_ADMIN_AUTH -o userdev | jq '.[].properties += [{"name":"location","value":"buildingA"}]' | jq '.[].properties' )}"
if ! echo "$new_deployment_props" | hzn ex dep updatepolicy bp_pws -u $USERDEV_ADMIN_AUTH -o userdev -f-; then
...
if ! echo "$new_service_pol" | hzn ex service addpolicy e2edev@somecomp.com/bluehorizon.network-services-netspeed_2.3.0_amd64 -u $E2EDEV_ADMIN_AUTH -o "e2edev@somecomp.com" -f-; then
...
if ! echo "$new_node_pol" | hzn policy update -f-; then
