...
list_output=$(hzn exchange user list -o "$HZN_ORG_ID" -u "$HZN_EXCHANGE_USER_AUTH" 2>&1)
echo "$list_output" | jq . >/dev/null 2>&1 || fail_exit "user list did not return valid JSON"
echo -e "$PREFIX list ok"

# Create API key
create_output=$(hzn exchange user createkey "$USERNAME" "$KEY_DESC" -o "$HZN_ORG_ID" -u "$HZN_EXCHANGE_USER_AUTH" 2>&1)
...
get_output=$(hzn exchange user getkey "$USERNAME" "$KEY_ID" -o "$HZN_ORG_ID" -u "$HZN_EXCHANGE_USER_AUTH" 2>&1)
...
# Delete API key
hzn exchange user removekey "$USERNAME" "$KEY_ID" -f -o "$HZN_ORG_ID" -u "$HZN_EXCHANGE_USER_AUTH" >/dev/null 2>&1 || fail_exit "removekey failed"
echo -e "$PREFIX remove ok"

# Confirm deletion
check_output=$(hzn exchange user getkey "$USERNAME" "$KEY_ID" -o "$HZN_ORG_ID" -u "$HZN_EXCHANGE_USER_AUTH" 2>&1 || true)
