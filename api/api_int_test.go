...
func setup() (string, *bolt.DB, error) {
	dir, err := os.MkdirTemp("", "api-attribute-")
	if err != nil {
		return "", nil, err
	}
...
-----END CERTIFICATE-----`)))
	...
	_, err = persistence.SaveNewExchangeDevice(db, "device-22", "tokenval", "Device 22", "device", "myorg", ".*", persistence.CONFIGSTATE_CONFIGURING, persistence.SoftwareVersion{persistence.AGENT_VERSION: "1.0.0"})
	assert.Nil(suite, err)
