...
func Test_DeleteHorizonDevice_success(t *testing.T) {
	...
	_, err = persistence.SaveNewExchangeDevice(db, *device.Id, *device.Token, *device.Name, "cluster", false, *device.Org, *device.Pattern, persistence.CONFIGSTATE_CONFIGURED)
	if err != nil {
...
func Test_DeleteHorizonDevice_fail1(t *testing.T) {
	...
	_, err = persistence.SaveNewExchangeDevice(db, *device.Id, *device.Token, *device.Name, "device", false, *device.Org, *device.Pattern, persistence.CONFIGSTATE_UNCONFIGURED)
	if err != nil {
		t.Errorf("unexpected error creating device %v", err)
	}
