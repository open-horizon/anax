...
// swagger:operation GET /secrets handleGetSecrets
//
// Get secrets.
//
...
func (api *SecretAPI) handleGetSecrets(writer http.ResponseWriter, request *http.Request) {
	glog.V(3).Infof(secAPILogString(fmt.Sprintf("GET /secrets")))
	...
	if authenticated, token, err := api.authenticator.Authenticate(request); !authenticated {
		glog.Errorf(secAPILogString(fmt.Sprintf("GET /secrets authenticate error: %v", err)))
		...
		if _, err := writer.Write(data); err != nil {
			glog.Errorf(secAPILogString(fmt.Sprintf("GET /secrets, failed to write to response body: %v", err)))
		}
	}
}
...
func (api *SecretAPI) handleSecrets(writer http.ResponseWriter, request *http.Request) {
	...
	if len(parts) == 0 {
		...
		// swagger:operation POST /secrets/{secretName}?received=true handleSecretReceived
		//
		...
		case http.MethodPost:
			...
			if receivedString != "" {
				...
				if received, err = strconv.ParseBool(receivedString); err != nil || !received {
					glog.Errorf(secAPILogString(fmt.Sprintf("POST /secrets/%s?received=%t, err: %v", secretName, received, err)))
					writer.WriteHeader(http.StatusBadRequest)
					return
				} else {
					// received == true
					glog.V(3).Infof(secAPILogString(fmt.Sprintf("POST /secrets/%s?received=true", secretName)))
		...
		// swagger:operation GET /secrets/{secretName} handleGetSecret
		//
		// Get a secret.
		//
		...
		case http.MethodGet:
			...
			glog.V(3).Infof(secAPILogString(fmt.Sprintf("GET /secrets/%s", secretName)))
